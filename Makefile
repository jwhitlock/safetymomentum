...
jslint:
	if type jslint >/dev/null 2>&1 ; then jslint safetymomentum/static/js/*; else echo "jslint not installed. To install node and jshint, use 'make install_jslint"; fi
...
coverage-console:
	coverage erase
	coverage run --source safetymomentum setup.py test
	coverage report -m
